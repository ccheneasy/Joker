...
export function useWatermark(parentEl = bodyEl) {
  ...
  const addMutationListener = (targetNode) => {
    ...
    // 创建观察器实例并传入回调
    // MutationObserver 是一个 JS API，用异步方式来监视 DOM 树中的节点插入、删除、属性更改等各种更改，并执行回调函数，不需要依赖于传统的事件监听机制。
  ...
  const addResizeListener = (targetNode) => {
    ...
    // 创建一个观察器实例并传入回调
    // ResizeObserver 是一个 JS API，用异步的方式来监视 DOM 元素的大小变化，并执行回调函数。它可以是监视单个元素，或者一个由多个元素组成的组，不需要轮询或依赖于传统的事件监听。
